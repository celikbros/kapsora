...
func TestDocumentRuleFixturePublishesOnlyItsPersonAndRetiresIdempotently(t *testing.T) {
	h := dbtest.New(t)
	tenant := h.CreateTenant("DOCUMENT_FIXTURE")
	...
	person := uuid.New()
	if _, err := ensureDocumentRule(ctx, svc, tenant, maker, maker, person, false); err == nil {
		t.Fatal("same maker/checker accepted")
	}
	if _, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, uuid.Nil, false); err == nil {
		t.Fatal("empty person accepted")
	}
	first, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, person, false)
	if err != nil {
		t.Fatalf("%#v", err)
	}
	...
	repeated, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, person, false)
	...
	// SubmitVersion ran both assertions: the target requires INVOICE; another person
	// remains approved. A failing case would have prevented publication entirely.
	rule, _ := documentRuleContent(person)
	if view.Rules[0].Condition != rule.Condition {
		t.Fatal("published predicate changed")
	}
	retired, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, person, true)
	if err != nil || retired.Status != "RETIRED" {
		t.Fatalf("retire = %+v, %v", retired, err)
	}
	again, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, person, true)
	if err != nil || again != retired {
		t.Fatalf("repeat retire = %+v, %v", again, err)
	}
	if _, err := ensureDocumentRule(ctx, svc, tenant, maker, checker, person, false); err == nil {
