...
func (s *Service) GetReport(ctx context.Context, rc identity.RequestContext, id uuid.UUID,
	...
	if errors.Is(err, ErrAccessPurposeRequired) {
		s.recordDenial(ctx, rc, person, domain.AggregateMedicalReport, id, req)
...
func (s *Service) CreateReport(ctx context.Context, rc identity.RequestContext,
	...
	err := s.withTx(ctx, rc, func(ctx context.Context, tx pgx.Tx) error {
		...
		// Read back rather than answering from the insert: the insert does not join the
		// case, so the record it returned does not know how sensitive the report is, and a
		// projection decided on a sensitivity nobody read would be a projection decided
		// wrongly.
		out, err = s.reloadReport(ctx, tx, rc, record.ID)
		return err
	})
	if err != nil {
		return ReportView{}, err
